...
export class FactionsApi extends runtime.BaseAPI {
    ...
    async getFactionRaw(requestParameters: GetFactionRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<GetFaction200Response>> {
        if (requestParameters.faction_symbol === null || requestParameters.faction_symbol === undefined) {
            throw new runtime.RequiredError('faction_symbol','Required parameter requestParameters.faction_symbol was null or undefined when calling getFaction.');
        }
        ...
        const response = await this.request({
            path: `/factions/{factionSymbol}`.replace(`{${"factionSymbol"}}`, encodeURIComponent(String(requestParameters.faction_symbol))),
            method: 'GET',
